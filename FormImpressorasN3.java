...
/*
 * 
 *ORDEM DE SERVIÇO SIMPRESS
Impressora N°: HCPR-PR-373
Anexo:
Sala:
Numero do Ticket: 5827955
Data de abertura: Horas: 14/01/2025 às 08:40
Numero Ticket GLPI: 2025011827
Conexão cabo () wifi()
Nível Critico Sim ( ) Não (X)
Backup Instalado Sim (X) Não ()
Redirecionado para outra Impressora Sim (X) Não () 
Se Sim!, para Qual Impressora: BKP1
Tecnico Responsável: 
 */
...
// classe implementa actionListener para eventos de botão
public class FormImpressorasN3 extends JPanel implements ActionListener{

	
	// Jlabels - Rótulos
	JLabel title; // titulo
	JLabel lblNumImpressora; // Nome da Impressora HCPR-PR
	JLabel lblNumOS; // Ordem de serviço Simpress
	JLabel LblNumGlpi; // Ticket Chamado GLPI
	JLabel lblConexao; // Tipo de Conexão
	JLabel lblCritico; // Nível Crítico
	JLabel lblBackup; // Backup Instalada?
	JLabel lblQualBackup; // Qual? (referente a backup)
	JLabel lblRedirect; // Redirecionada? 
	JLabel lblQualRedirect; // Qual? (referente a redirecionada)
	...
	JTextField txtNumOs; // número da OS Simpress
	JTextField txtNumGlpi; // número do chamado no GLPI
	JTextField txtQualBackup; // informar a backup instalada
	JTextField txtQualRedirect; // informar a impressora redirecionada
	JTextField txtSala; // informar o setor e a sala
	
	//JRadioButtons - botões de seleção do tipo rádio
	JRadioButton radioCabo, radioWifi; // Tipo de Conexão Cabo / Wifi
	JRadioButton radioCriticoS, radioCriticoN; // Nivel Crítico S/N
	JRadioButton radioBackupS, radioBackupN; // Backup Instalada S/N
	JRadioButton radioRedirectS, radioRedirectN; // Redirecionada S/N
	...
	// Cor Personalizada
	Color color2 = new Color(222,184,135); // cor Dark Khaki
	...
	ButtonGroup groupCritic = new ButtonGroup(); // grupo nível crítico (S/N)
	ButtonGroup groupBackup = new ButtonGroup(); // grupo backup instalada (S/N)
	ButtonGroup groupRedirect = new ButtonGroup(); // grupo redirecionada (S/N)
	ButtonGroup groupLocal = new ButtonGroup(); // grupo local (Hc / Casas Externas)
	
	// Botões
	JButton enviar; // enviar formulário pelo Teams (em implementação)
	...
	String nivelCritico; // escreve sim ou não (na seleção) dependendo do botão selecionado (referente a nível crítico)
	String backupInstalada; // escreve sim ou não (na seleção) dependendo do botão selecionado (referente a backup instalada)
	String redirecionada; // escreve sim ou não (na seleção) dependendo do botão selecionado (referente a redirecionada)
	...
	// Desenha todos os componentes do form no painel
	public void makeFields() {
		
		// configurações titulo
		title = new JLabel("Ordem de Serviço Simpress"); // instancia e coloca texto
		title.setFont(fontTitle); // fonte
		title.setBounds(100,15, 330, 40); // coordenadas e taanho
		...
		lblNumImpressora.setBounds(30,60, 350, 40);
		...
		txtNumImpressora.setBounds(260,60, 50, 30);
		this.add(txtNumImpressora);
		
		// Configurações label Número OS Simpress
		lblNumOS = new JLabel("Ordem de Serviço Simpress: #");
		lblNumOS.setFont(fontText);
		lblNumOS.setBounds(30,100, 300, 35);
		lblNumOS.setForeground(Color.black);
		this.add(lblNumOS);
		
		// Configurações caixa de texto Número da OS Simpress
		txtNumOs = new JTextField();
		txtNumOs.setFont(fontText);
		txtNumOs.setBounds(260,100, 75, 30);
		this.add(txtNumOs);
		...
		// Configurações label tipo de conexão
		lblConexao = new JLabel("Tipo de Conexão:");
		lblConexao.setFont(fontText);
		lblConexao.setBounds(30,182, 300, 35);
		lblConexao.setForeground(Color.black);
		this.add(lblConexao);
		
		// Configurações radio button Cabo
		radioCabo = new JRadioButton("Cabo");
		radioCabo.setFont(fontText);
		radioCabo.setBounds(180,180, 70, 35);
		radioCabo.setBackground(color2);
		radioCabo.setForeground(Color.black);
		radioCabo.addActionListener(this);
		groupConection.add(radioCabo);
		this.add(radioCabo);
		
		// Configurações radio button Wifi
		radioWifi = new JRadioButton("Wifi");
		radioWifi.setFont(fontText);
		radioWifi.setBounds(250,180, 100, 35);
		radioWifi.setBackground(color2);
		radioWifi.setForeground(Color.black);
		radioWifi.addActionListener(this);
		groupConection.add(radioWifi);
		this.add(radioWifi);
		
		// Configurações label crítico
		lblCritico = new JLabel("Nível Crítico: ");
		lblCritico.setFont(fontText);
		lblCritico.setBounds(30,222, 300, 35);
		lblCritico.setForeground(Color.black);
		this.add(lblCritico);
		
		// Configurações radio button crítico sim
		radioCriticoS = new JRadioButton("Sim");
		radioCriticoS.setFont(fontText);
		radioCriticoS.setBounds(180,220, 70, 35);
		radioCriticoS.setBackground(color2);
		radioCriticoS.setForeground(Color.black);
		radioCriticoS.addActionListener(this);
		groupCritic.add(radioCriticoS);
		this.add(radioCriticoS);
		
		// Configurações radio button crítico não
		radioCriticoN = new JRadioButton("Não");
		radioCriticoN.setFont(fontText);
		radioCriticoN.setBounds(250,220, 70, 35);
		radioCriticoN.setBackground(color2);
		radioCriticoN.setForeground(Color.black);
		radioCriticoN.addActionListener(this);
		groupCritic.add(radioCriticoN);
		this.add(radioCriticoN);
		
		// configurações label backup
		lblBackup = new JLabel("Backup Instalada?");
		lblBackup.setFont(fontText);
		lblBackup.setBounds(30,260, 300, 35);
		lblBackup.setForeground(Color.black);
		this.add(lblBackup);
		
		// Configurações radio button backup sim
		radioBackupS = new JRadioButton("Sim");
		radioBackupS.setFont(fontText);
		radioBackupS.setBounds(180,260, 60, 35);
		radioBackupS.setBackground(color2);
		radioBackupS.setForeground(Color.black);
		radioBackupS.addActionListener(this);
		groupBackup.add(radioBackupS);
		this.add(radioBackupS);
		
		// Configurações radio button backup não
		radioBackupN = new JRadioButton("Não");
		radioBackupN.setFont(fontText);
		radioBackupN.setBounds(250,260, 60, 35);
		radioBackupN.setBackground(color2);
		radioBackupN.setForeground(Color.black);
		radioBackupN.addActionListener(this);
		groupBackup.add(radioBackupN);
		this.add(radioBackupN);
		
		// configurações label qual referente a backup
		lblQualBackup = new JLabel("Qual? ");
		lblQualBackup.setFont(fontText);
		lblQualBackup.setBounds(310,260, 300, 35);
		lblQualBackup.setVisible(false);
		lblQualBackup.setForeground(Color.black);
		this.add(lblQualBackup);
		
		// configurações caixa de texto JTextField para qual referente a backup
		txtQualBackup = new JTextField();
		txtQualBackup.setFont(fontText);
		txtQualBackup.setBounds(380,260, 60, 30);
		txtQualBackup.setVisible(false);
		this.add(txtQualBackup);
		
		// configurações label para Redirecionada
		lblRedirect = new JLabel("Redirecionada?");
		lblRedirect.setFont(fontText);
		lblRedirect.setBounds(30,300, 350, 35);
		lblRedirect.setForeground(Color.black);
		this.add(lblRedirect);
		
		// Configurações radio button redirecionada sim
		radioRedirectS = new JRadioButton("Sim");
		radioRedirectS.setFont(fontText);
		radioRedirectS.setBounds(180,300, 60, 35);
		radioRedirectS.setBackground(color2);
		radioRedirectS.setForeground(Color.black);
		radioRedirectS.addActionListener(this);
		groupRedirect.add(radioRedirectS);
		this.add(radioRedirectS);
		
		// Configurações radio button redirecionada não
		radioRedirectN = new JRadioButton("Não");
		radioRedirectN.setFont(fontText);
		radioRedirectN.setBounds(250,300, 60, 35);
		radioRedirectN.setBackground(color2);
		radioRedirectN.setForeground(Color.black);
		radioRedirectN.addActionListener(this);
		groupRedirect.add(radioRedirectN);
		this.add(radioRedirectN);
		this.add(radioRedirectN);
		
		// configurações label Qual referente a Redirecionada
		lblQualRedirect = new JLabel("P/ Qual? ");
		lblQualRedirect.setFont(fontText);
		lblQualRedirect.setBounds(315,300, 300, 35);
		lblQualRedirect.setVisible(false);
		lblQualRedirect.setForeground(Color.black);
		this.add(lblQualRedirect);
		
		// configurações caixa de texto JTextField para qual referente a redirecionada
		txtQualRedirect = new JTextField();
		txtQualRedirect.setFont(fontText);
		txtQualRedirect.setBounds(380,300, 60, 30);
		txtQualRedirect.setVisible(false);
		this.add(txtQualRedirect);
		
		// configurações label técnico
		lblTecnico = new JLabel("Técnico Responsável: ");
		lblTecnico.setFont(fontText);
		lblTecnico.setBounds(30,340, 350, 35);
		...
		comboBoxTecnico.setBounds(190, 340, 270, 30); // sets coordinates and size
		...
		lblData.setBounds(30,380, 350, 35);
		...
		lblHora.setBounds(250,380, 350, 35);
		...
		copiar.setBounds(70, 550, 100, 35);
		copiar.setFont(fontText);
		copiar.setForeground(Color.black);
		copiar.addActionListener(this);
		this.add(copiar);
		
		// configurações botão enviar
		enviar = new JButton("Enviar");
		enviar.setBounds(200, 550, 100, 35);
		enviar.setFont(fontText);
		...
		limpar.setBounds(330, 550, 100, 35);
	...
	// Habilita campos de localização de Casas Externas
	...
	public void checkFields() {
		...
		if (isHc && isCExt == false) {
			...
			if  (
				...
				|| (txtNumOs.getText().isEmpty()) 
				|| (txtNumGlpi.getText().isEmpty())
				|| (!radioCabo.isSelected() && (!radioWifi.isSelected()))
				|| (!radioCriticoS.isSelected() && (!radioCriticoN.isSelected()))
				|| (!radioBackupS.isSelected() && (!radioBackupN.isSelected()))
				|| (radioBackupS.isSelected() && (txtQualBackup.getText().isEmpty()))
				|| (!radioRedirectS.isSelected() && (!radioRedirectN.isSelected()))
				|| (radioRedirectS.isSelected() && (txtQualRedirect.getText().isEmpty()))
	...
	// Copia texto do form para a área de transferência (Ctrl + C)
 	public void copy() { 
             ...
             + "\n" + lblNumOS.getText() + txtNumOs.getText()
             + "\n" + LblNumGlpi.getText() + txtNumGlpi.getText()
             + "\n" + tipoConexao
             + "\n" + lblCritico.getText() + nivelCritico
             + "\n" + lblBackup.getText() + backupInstalada + txtQualBackup.getText()
             + "\n" + lblRedirect.getText() + redirecionada + txtQualRedirect.getText()
	...
	public void reset() {
		...
		groupBackup.clearSelection();
		groupConection.clearSelection();
		groupCritic.clearSelection();
		groupLocal.clearSelection();
		groupRedirect.clearSelection();
		
		// Limpa texto dos campos de texto JFields
		txtNumGlpi.setText("");
		txtNumImpressora.setText("");
		txtNumOs.setText("");
		txtQualBackup.setText("");
		txtQualRedirect.setText("");
		txtSala.setText("");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (radioCabo.isSelected()) { // se o botão radio do cabo (grupo de conexões) for selecionado
			tipoConexao = "Tipo de conexão: Cabo"; // escrever texto na String
		}
		
		if (radioWifi.isSelected()) { // se o botão radio de wifi (grupo de conexões) for selecionado
			tipoConexao = "Tipo de conexão: Wifi"; // escrever texto na String
		}
		
		if (radioCriticoS.isSelected()) { // se o botão radio Sim de nível crítico  for selecionado
			nivelCritico = " Sim"; // escrever texto na String
		}
		
		if (radioCriticoN.isSelected()) { // se o botão radio Nãode nível crítico  for selecionado
			nivelCritico = " Não";
		}
		
		
		if (radioBackupS.isSelected()) {
			lblQualBackup.setVisible(true);
			txtQualBackup.setVisible(true);
			backupInstalada = " Sim Qual? ";
		}
		
		if (radioBackupN.isSelected()) {
			lblQualBackup.setVisible(false);
			txtQualBackup.setVisible(false);
			backupInstalada = " Não";
			txtQualBackup.setText("");
		...
		if (radioRedirectS.isSelected()) {
			lblQualRedirect.setVisible(true);
			txtQualRedirect.setVisible(true);
			redirecionada = " Sim Qual? ";

		}
		
		if (radioRedirectN.isSelected()) {
			lblQualRedirect.setVisible(false);
			txtQualRedirect.setVisible(false);
			redirecionada = " Não";
			txtQualRedirect.setText("");
